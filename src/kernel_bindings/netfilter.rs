    ...
    unsafe {
        match hook_func_rust {
            Some(func) => {
                ...
                // if((*header).protocol == net::ip_protocol::TCP) {
                //     let tcp_in_buff = ((&buffer).as_ptr() as usize + 20 )as *const bindings::tcphdr;
                //     let tcp_header = bindings::tcp_hdr_wrapped(skb);
                //     use crate::println;
                //     println!("TCP {} -> {} / {} -> {}", (*tcp_header).source, (*tcp_header).dest, (*tcp_in_buff).source, (*tcp_in_buff).dest);
                // }
...
fn test(packet: net::IPv4Packet) -> net::IPv4Packet {
    use crate::println;
    // println!("{}", packet.payload[packet.payload.len() - 1]);
    packet
}
